...
public class DetteServiceImpl implements DetteService {
    ...
    public List<Dette> listerDemandesDette(String etat) {
        return detteRepository.selectAll().stream()
                .filter(dette -> dette.getEtat().equals(etat))
                .collect(Collectors.toList());
    }

    public void createDetteForClientWithArticlesAndPayment(Client client, List<Article> articles, Double montant, Double montantVerser, Date date, Double montantPaiement, Double prixVente, Integer qteVendu ) {

    
        Dette dette = new Dette(client, qteVendu, qteVendu, date, articles);
