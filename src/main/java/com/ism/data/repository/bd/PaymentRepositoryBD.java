...
public class PaymentRepositoryBD extends RepositoryBDImpl<Payment> implements PaymentRepository {
    ...
    public PaymentRepositoryBD() {
        this.tableName = "payments"; // Assurez-vous que le nom de la table est correct
    }
    ...
    public void insert(Payment payment) {
        try {
            ...
            this.ps.setTimestamp(2, new Timestamp(payment.getDate().getTime())); // Conversion correcte
            this.ps.setInt(3, payment.getDette().getId()); // Assurez-vous que la relation est bien établie

            this.executeUpdate();
            ResultSet rs = this.ps.getGeneratedKeys();
            if (rs.next()) {
                payment.setId(rs.getInt(1)); // Met à jour l'ID du paiement
            }
            rs.close();
    ...
    public Payment convertToObject(ResultSet rs) throws SQLException {
        ...
        payment.setDate(rs.getTimestamp("date")); // Récupère le Timestamp et le passe directement
        // Remplir le champ Dette si nécessaire
        // payment.setDette(...); // Vous pouvez récupérer la Dette ici si nécessaire
        return payment;
    }
    ...
    public void update(Payment payment) {
        try {
            ...
            this.ps.setTimestamp(2, new Timestamp(payment.getDate().getTime())); // Conversion correcte
            this.ps.setInt(3, payment.getDette().getId()); // Assurez-vous que la relation est bien établie
            this.ps.setInt(4, payment.getId()); // ID du paiement à mettre à jour
